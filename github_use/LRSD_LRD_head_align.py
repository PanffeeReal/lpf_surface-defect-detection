...
class FSAFHead_simple_SS_Align(RetinaHead):
    ...
    def forward_single(self, x):
        ...
        ##
        ##  here we need to clarify where to plug in the self.feature_alignment: before cls_branch or after  !!
        ##
        if self.use_feature_alignment:
            cls_feat = self.feature_alignment(cls_feat,torch.log(bbox_pred.exp()))  # bbox_pred.exp() or bbox_pred()
...
#                 print('notencode')
            ...
            if self.level_select == True:
                ...
                centerness_targets = self.centerness_target(
                    pos_anchors, bbox_targets[pos_inds, :] )
                # print('TTTTTTTTTTTTTTTTTTT')
                bbox_weights[pos_inds,:] = centerness_targets.unsqueeze(1)
                label_weights[pos_inds]=centerness_targets.unsqueeze(1)
            #     print(pos_anchors.shape,bbox_weights[pos_inds,:].shape,label_weights[pos_inds].shape)
    ...
    def loss(self,
        ...
        # print("xxx:",labels_list[0].shape,labels_list[1].shape,labels_list[2].shape)
        # print("xxxxxxxxx:",label_weights_list[0].shape,label_weights_list[1].shape,label_weights_list[2].shape)    ##(batch,num_samples,80)
        # print("xxxxxxxxx:",bbox_targets_list[0].shape,bbox_targets_list[1].shape,bbox_targets_list[2].shape)    ##(batch,num_samples,4)
        ...
        # print("head:" ,len(losses_cls),losses_cls[0].shape,losses_cls[1].shape,losses_cls[2].shape)   #5levels::::(num_samples,80)
        # print("head22:" ,len(losses_bbox),losses_bbox[0].shape,losses_bbox[1].shape,losses_bbox[2].shape)   #5levels::::(num_samples)
...
#=======================
        ...
        pos_recall = self.calculate_pos_recall(cls_scores, labels_list,
                                               pos_inds)
        #cal weight_second sum
        sum_reweight=torch.zeros(1).cuda()
        for abc in second_wei_sum:
            # print('xxxxxxxxxx',abc.squeeze().shape,sum_reweight.shape)
            sum_reweight += abc
        # print(sum_reweight.shape,sum_reweight)
        # print('yyyyy',sum_reweight)
        #
        ...
        if num_pos == 0:  # No gt
            ...
            # avg_factor = num_pos
            avg_factor = sum_reweight.squeeze()
#=========================

        for i in range(len(losses_cls)):
            # print(losses_cls[i].shape,losses_cls[i])
...
#         return loss_cls, loss_bbox  ##,loss_centerness
    ...
    def centerness_target(self, anchors, gts):
        ...
        return centerness
    def reweight_loss_single(self, cls_loss, reg_loss,label_weights,reg_weights, assigned_gt_inds,
        ...
        """
        #because of SS's weights ,here needs to reload the weight : lpf
        loc_weight = torch.ones_like(reg_loss)
        cls_weight = torch.ones_like(cls_loss)

        ##  (B,num.)
        #wrong:::::because weights had been used in loss_single()  .no need to repeatly use
#         cls_weight=label_weights.reshape(-1, self.num_classes)
#         loc_weight=reg_weights.reshape(-1, 4)[:,0]
        #lpf
        ...
        weight_second_sum=torch.zeros(1).cuda()
        weight_third_sum=torch.zeros(1).cuda()
        weight_first_sum=torch.zeros(1).cuda()
#=======================================================
...
#find zeroing_indices and neg_indices
           # zeroing_indices = zeroing_indices    #  pos samples need to be weight by 0   shape:(num_neg,)    type : bool
            ...
            weight_first_sum=reg_weights.reshape(-1, 4)[:,0][pos_indices[zeroing_indices==False]].sum()
            ##calculate weight by each_gt's loss
            #(3,num_gts)-->each gt's weight to 3 selected levels
            # alpha=third_index.new_full((1,),0.5)
#             weight_first = loc_weight[pos_indices[zeroing_indices == False]]
            alpha=self.alpha
            # print('****************************************')
            # print(alpha)
            ...
            weight_second=(weight_tensor[1,assigned_gt_inds[second_index]]) #line-->array
#             print('===========================')
#             # print(loss_levels_lpf.shape,weight_tensor.shape)
#             # # print(weight_tensor)
#             print(weight_second.shape)
            weight_third=(weight_tensor[2,assigned_gt_inds[third_index]]) ##line --> array
...
#lpf
                ...
                # # print(loc_weight.shape,cls_weight.shape)
                # # print(loc_weight[second_index].shape,weight_second.shape)
                # # print('fisrt_cls_weight',loc_weight[pos_indices[zeroing_indices==False]])
                # # print('first_loc_weight',cls_weight[pos_indices[zeroing_indices==False]])
                # print(loc_weight[second_index].shape,weight_second.shape)
                ...
                cls_weight[neg_indices, zeroing_labels] = 0
                # print('xxxxxxxxxxxxx')
                # print(cls_weight[second_index, second_labels].shape)
                # print(cls_weight[third_index, third_labels].shape)
                ...
                weight_second_tmp= weight_second * reg_weights.reshape(-1, 4)[:,0][second_index]
                weight_third_tmp= weight_third * reg_weights.reshape(-1, 4)[:,0][third_index]

                weight_second_sum= weight_second_tmp.sum()
                weight_third_sum=weight_third_tmp.sum()
        ...
        #
        # print('loss cal',cls_loss.shape,cls_weight.shape)
        # print('REGRESS cal',reg_loss.shape,loc_weight.shape)

        cls_loss = weight_reduce_loss(cls_loss, cls_weight, reduction='sum')
        reg_loss = weight_reduce_loss(reg_loss, loc_weight, reduction='sum')


        return cls_loss, reg_loss, pos_flags ,weight_first_sum+weight_second_sum+weight_third_sum
    #lpf
